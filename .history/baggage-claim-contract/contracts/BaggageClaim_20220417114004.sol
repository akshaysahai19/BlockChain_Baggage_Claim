...
contract BaggageClaim{
    ...
    function registerAirports(string memory airportsName, uint pricePerbag) public{
        ...
        airportNames.push(Airports(address(newAirports), airportsName));
    }
    ...
    function settlePayment(uint index, uint hashOfDetails) public payable onlyCustomer{
        address selectedAirport = airportNames[index].airportAddress;
        address payable toAirport = address(uint160(selectedAirport));
