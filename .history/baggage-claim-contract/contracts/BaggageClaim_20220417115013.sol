...
contract BaggageClaim{
    ...
    function registerAirports(string memory airportsName, uint pricePerbag) payable public{
        ...
        airportNames.push(Airports(payable(newAirports), airportsName));
    }
    ...
    function settlePayment(uint index, uint hashOfDetails) public payable onlyCustomer{
        address airportAddress = airportNames[index].airportAddress;
        address payable toAirport = payable(address(airportAddress));
