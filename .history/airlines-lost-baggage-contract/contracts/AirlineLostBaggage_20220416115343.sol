...
contract AirlineLostBaggage{
    ...
    modifier onlyAirline(address airlineAddress){
        require(airlineBaggage[airlineAddress].memebershipStatus==1);
        _;
    }
    ...
    function registerAirlines(string memory airlinesName, uint pricePerbag) public payable{
        ...
        balanceDetails[newAirlines].escrow = msg.value;
    }

    function unregisterAirlines(address payable toAirlines) public  onlyAirlineAuthority onlyAirline(toAirlines){
    ...
    function addBaggaage(uint noOfBaggage) public onlyAirline(msg.sender){
        airlineBaggage[msg.sender].totalBaggage+=noOfBaggage;
        airlineBaggage[msg.sender].totalUnclaimedBaggage+=noOfBaggage;

    }

    function requestToClaimBaggage(address fromAirline, uint hashOfDetails, uint noOfBaggage) public payable notAnAirline onlyAirline(fromAirline){
    ...
    function responseToClaimBaggage(uint done, uint hashOfDetails) public onlyAirline(msg.sender){
