...
contract AirlineLostBaggage{
    ...
    constructor () payable public{
        ...
        airlineBaggage[msg.sender].memebershipStatus = 2;
        myBalance = address(this).balance;
    }

    event LogMessage(string message, uint value);

    function registerAirlines(string memory airlinesName, uint pricePerbag) public notAnAirline{
        ...
        balanceDetails[newAirlines].escrow = 0;
        airlineNames.push(Airlines(newAirlines, airlinesName));
    }
    ...
    function requestToClaimBaggage(address fromAirline, uint hashOfDetails, uint noOfBaggage) public payable notAnAirline{
        uint totalAmountNeeded = (airlineBaggage[fromAirline].pricePerbag)*noOfBaggage;
    ...
    function getAirlinesCount() public view returns(uint){
        return airlineNames.length;
    }

    function getAirlinesName(uint index) public view returns(string memory){
        return airlineNames[index].airlineName;
    }

    function getAirlinesAddress(uint index) public view returns(address){
        return airlineNames[index].airlineAddress;
    }

    function getAirlineMembershipStatus() public view returns(uint){
        return airlineBaggage[msg.sender].memebershipStatus;
    }

    function removeAirlinesFromList(address payable toAirlines) private onlyAirlineAuthority{
        if(airlineBaggage[toAirlines].memebershipStatus!=1){
            revert();
        }
        for(uint i=0;i<airlineNames.length;i++){
            if(airlineNames[i].airlineAddress == toAirlines){
                airlineNames.remove(i);
            }
        }
    }
