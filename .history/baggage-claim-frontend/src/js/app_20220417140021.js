App = {
  ...
  accountType:0,  //0-customer, 1-airport
  currentAccount: null,
  init: function () {
    return App.initWeb3();
  },
    ...
    airportsInstance
                  ...
                  .then((data) => {
                    ...
                    airportsInstance.getAirportPricePerBag( { from: address }).then((data) => {
                      ...
                      console.log("index = "+index, "name = ", name, "address = ", address, "price = ", price);
                      App.airports.push({name: name,
                        address: address,
                        price: price});
                        if(index == count-1){
                          console.log("App.airports FinalList = ", App.airports);
  ...
  bindEvents: function () {
    ...
    $(document).on("click", "#airportTypeButton", App.handleBalance); //add baggage
    $(document).on("click", "#customerTypeButton", App.handleBalance); //add baggage
  },
        ...
        .then(function (result, err) {
          if (result) {
            ...
            if (parseInt(result.receipt.status) == 1) {
              ...
              App.airports.push({ name: airportsName, address: App.account, price: pricePerBaggage });
              console.log(App.airports);
              App.handleUpdateAirportsList();
              App.checkMemeberShipStatus();
            } else
  ...
  handleSettlePayment: function (index, hashOfDetails) {
    ...
    .then(function (instance) {
      ...
      console.log("App.airports[index].address ", App.airports[index].address);
        ...
        .then(function (result, err) {
          if (result) {
            ...
            if (parseInt(result.receipt.status) == 1) {
              ...
              alert(App.account + " Airports unregistration failed due to revert");
          } else {
