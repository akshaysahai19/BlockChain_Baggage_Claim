App = {
  ...
  bindEvents: function() {
    ...
    $(document).on('click', '#pay', function(){ var sellingAirportWhileSettle = $('#sellingAirportWhileSettle').val();var hashDetailsWhileSettle = $('#hashDetailsWhileSettle').val(); App.handleSettlePayment(sellingAirportWhileSettle, hashDetailsWhileSettle); });//claim response
    $(document).on('click', '#airportUnregister', function(){ var unregisterAirportName = $('#unregisterAirportName').val(); var perPriceBag = $('#perPriceBag').val();App.handleRegistration(airportname,perPriceBag); });//register airlines
  ...
  handleRequestBaggage: function(sellingAirline, bagCount, hashDetails) {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.claim.deployed().then(function(instance) {
        ...
        return airlinesRequestInstance.requestToClaimBaggage(sellingAirline, hashDetails, bagCount,{from: account});
      }).then(function(result, err){
            if(result){
              console.log("result: %o", result);
                console.log(result.receipt.status);
