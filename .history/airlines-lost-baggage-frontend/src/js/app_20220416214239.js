  ...
  initContract: function() {
    ...
    App.contracts.claim.deployed().then(function(instance) {
      ...
      for(var i = 0; i < airlinesCount; i++){
              ...
              airlinesInstance.getAirlineMembershipStatus(address, {from: App.account}).then(data=>{
                if(data==1){
    ...
    console.log("App.contracts.claim.currentProvider: " + App.contracts.claim.currentProvider);
...
// For now, 'eth_accounts' will continue to always return an array
  ...
  bindEvents: function() {
    ...
    $(document).on('click', '#claimBaggageResponse', function(){ var totalClaimBaggageNumber = $('#totalClaimBaggageNumber').val();var hashDetailsWhileClaim = $('#hashDetailsWhileResponse').val(); App.handleRequestBaggage(sellingAirportWhileClaim, totalClaimBaggageNumber, hashDetailsWhileClaim); });//claim response
  ...
  screenFilter: function(memeberShip){
    if(memeberShip==0){
      ...
      $("#airlineListCard").css("display", "none");
