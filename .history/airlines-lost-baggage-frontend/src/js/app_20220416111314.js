App = {
  ...
  handleRegistration: function(event) {
    ...
    // var airlinesName = parseInt($(event.target).data('id'));//register airlinesName
    // var pricePerBaggage = parseInt($('#pricePerBaggage').val());//price per bag
        ...
        return airlinesRegistryInstance.registerAirlines(airlinesName, pricePerBaggage, {from: account});
            ...
            if(result){
              console.log("result: %o", result);
                ...
                alert(account + " Airlines registration not done successfully due to revert")
            } else {
        ...
        return airlinesRegistryInstance.addBaggaage(bagCount, 0,{from: account});
            ...
            if(result){
              console.log("result: %o", result);
                ...
                alert(account + " Baggage not added successfully due to revert")
            } else {
  ...
  handleRequestBaggage: function(event) {
    ...
    // var airlinesName = parseInt($(event.target).data('id'));//register airlinesName
    // var pricePerBaggage = parseInt($('#pricePerBaggage').val());//price per bag
    var purchaseFrom = "";//pass these values from the UI
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.claim.deployed().then(function(instance) {
        ...
        return airlinesRegistryInstance.requestToClaimBaggage(purchaseFrom, hashOfDetails, buyQuantity,{from: account});
      }).then(function(result, err){
            if(result){
              console.log("result: %o", result);
                console.log(result.receipt.status);
                ...
                alert(account + " Baggage Request not done successfully due to revert")
            } else {
                alert(account + " Baggage add failed")
            }   
        });
