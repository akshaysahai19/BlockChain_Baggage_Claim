App = {
  ...
  initContract: function() {
     await $.getJSON('AirlineLostBaggage.json', function(data) {
    ...
    App.contracts.claim.deployed().then(function(instance) {
      ...
      var airlinesCount = airlinesInstance.getAirlinesCount({from: App.account});
      for(var i = 0; i < airlinesInstance.getAirlinesCount({from: App.account}); i++){
        airlinesInstance.getAirport(App.names[i]).then(function(airport){
          App.airports.push(airport);
        });
      }
    }).then(function(res){
    console.log(res);
  ...
  handleRegistration: function(airlinesName,pricePerBaggage) {
    ...
    for (i = 0; i < App.airport.length; i ++) {
      if(App.airport[i] == airlinesName){
